...
import sqlite3
from .sqlit import info_members, reg_one_channel, reg_channels,del_one_channel,cheak_traf,obnovatrafika,info_chyornaya_vdova,info_good_film1,info_films_online_everyday,reg_partners_schet,cheach_all_par,info,reg_utm_support,cheak_support,changee_support,regviplata,cheak_viplats
...
ADMIN_ID =[ADMIN_ID_1,ADMIN_ID_2, ADMIN_ID_4,ADMIN_ID_3,MODERN_ID_5,]
    ...
    if call.message.chat.id in ADMIN_ID:
        ...
        for i in answer:
            ...
            await bot.send_message(chat_id=call.message.chat.id, text=f'<b>Канал:</b> {i[0]}\n'
                                                                      ...
                                                                      f'<b>Трафика всего:</b> {i[2]}',parse_mode='html')
...
async def get_reg_support(message: types.Message, state: FSMContext):
    if message.chat.id in ADMIN_ID:
        try:
            await state.update_data(utm=message.text)
...
async def get_reg_support2(message: types.Message, state: FSMContext):
    if message.chat.id in ADMIN_ID:
        try:
            y = await state.get_data()
            utm = y['utm']
            reg_utm_support(utm=utm, info= message.text)
